...
const Acanthus = ({ x, y, w, h, opacity = 0.7 }: LeafProps) => {
  ...
  return (
    <g transform={`translate(${x} ${y})`}>
      <path pathLength={1} className="draw" d={half} fill={MASS} strokeOpacity={opacity} />
      <path pathLength={1} className="draw" d={half} fill={MASS} strokeOpacity={opacity * 0.82} transform="scale(-1 1)" />
        ...
        <g key={side} strokeOpacity={opacity * (side === -1 ? 0.5 : 0.4)}>
          ...
          {LOBES.map(({ out, back }, i) => (
            <g key={i}>
              ...
              />
              {i < LOBES.length - 1 && (
                <circle pathLength={1} className="draw"
  ...
  return (
    <g mask={`url(#${maskId})`}>
      ...
      {arrises(spec).map(({ key, x1, y1, x2, y2, opacity, width }) => (
        <line pathLength={1} className="draw" key={key} x1={x1} y1={y1} x2={x2} y2={y2} strokeOpacity={opacity} strokeWidth={width} />
      ))}
      {/* Silhouette: the left edge catches the light, the right falls away. */}
      <line pathLength={1} className="draw" x1={CX - rTop} y1={top} x2={CX - rBot} y2={COLUMN_H} strokeOpacity={0.9} />
      <line pathLength={1} className="draw" x1={CX + rTop} y1={top} x2={CX + rBot} y2={COLUMN_H} strokeOpacity={0.62} />
    </g>
  );
};
...
const DoricCapital = () => (
  <g>
    ...
    <rect pathLength={1} className="draw" x={CX - 56} y={0} width={112} height={16} fill={MASS} strokeOpacity={0.85} />
    <line pathLength={1} className="draw" x1={CX - 56} y1={5} x2={CX + 56} y2={5} strokeOpacity={0.3} />
    <line pathLength={1} className="draw" x1={CX - 52} y1={16} x2={CX + 52} y2={16} strokeOpacity={0.3} />
    {/* Echinus — a short ovolo flaring out from the necking to the abacus */}
    <path pathLength={1} className="draw" d={`M ${CX - 56} 16 C ${CX - 56} 25 ${CX - 51} 34 ${CX - 41} 34`} strokeOpacity={0.85} />
    <path pathLength={1} className="draw" d={`M ${CX + 56} 16 C ${CX + 56} 25 ${CX + 51} 34 ${CX + 41} 34`} strokeOpacity={0.6} />
    <path pathLength={1} className="draw" d={`M ${CX - 50} 17 C ${CX - 50} 24 ${CX - 46} 30 ${CX - 38} 30`} strokeOpacity={0.24} />
    {/* Annulets over the necking */}
    {[34, 38, 42].map((y) => (
      <line pathLength={1} className="draw" key={y} x1={CX - 41} y1={y} x2={CX + 41} y2={y} strokeOpacity={y === 34 ? 0.6 : 0.35} />
    ))}
  </g>
);
...
const IonicCapital = () => {
  ...
  return (
    <g>
      <rect pathLength={1} className="draw" x={CX - 60} y={0} width={120} height={11} fill={MASS} strokeOpacity={0.8} />
      <line pathLength={1} className="draw" x1={CX - 60} y1={3.5} x2={CX + 60} y2={3.5} strokeOpacity={0.3} />
      ...
      <path pathLength={1} className="draw" d={`M ${CX - 27} 11 C ${CX - 29} 30 ${CX - 22} 44 ${CX - 14} 50`} strokeOpacity={0.4} />
      <path pathLength={1} className="draw" d={`M ${CX + 27} 11 C ${CX + 29} 30 ${CX + 22} 44 ${CX + 14} 50`} strokeOpacity={0.3} />
      {[-16, 0, 16].map((dx) => (
        <g key={dx} transform={`translate(${CX + dx} 28)`}>
          <ellipse pathLength={1} className="draw" rx={7} ry={11} fill={MASS} strokeOpacity={0.5} />
          <ellipse pathLength={1} className="draw" rx={4} ry={7} strokeOpacity={0.3} />
        </g>
      ))}
      {[-8.5, 8.5].map((dx) => (
        <path pathLength={1} className="draw" key={dx} d={`M ${CX + dx} 19 L ${CX + dx} 38 M ${CX + dx - 2.5} 22 L ${CX + dx} 19 L ${CX + dx + 2.5} 22`} strokeOpacity={0.3} />
      ))}
      ...
      {([-1, 1] as const).map((side) => {
        ...
        return (
          <g key={side} strokeOpacity={near ? 0.8 : 0.55}>
            <path pathLength={1} className="draw" d={spiral(cx, eyeY, 24, 3.2, 2.5, dir)} />
            <path pathLength={1} className="draw" d={spiral(cx, eyeY, 17, 2.4, 2.5, dir)} strokeOpacity={near ? 0.5 : 0.35} />
            <circle pathLength={1} className="draw" cx={cx} cy={eyeY} r={2.6} fill={MASS} strokeOpacity={0.7} />
          </g>
        );
      })}

      {/* Astragal under the capital */}
      <line pathLength={1} className="draw" x1={CX - 40} y1={54} x2={CX + 40} y2={54} strokeOpacity={0.55} />
      <line pathLength={1} className="draw" x1={CX - 40} y1={58} x2={CX + 40} y2={58} strokeOpacity={0.35} />
    </g>
  );
};
...
const CorinthianCapital = () => (
  <g>
    ...
    <path
      ...
      stroke="none"
    />
    <path pathLength={1} className="draw" d={`M ${CX - 40} 104 C ${CX - 46} 76 ${CX - 54} 44 ${CX - 57} 16`} strokeOpacity={0.35} />
    <path pathLength={1} className="draw" d={`M ${CX + 40} 104 C ${CX + 46} 76 ${CX + 54} 44 ${CX + 57} 16`} strokeOpacity={0.25} />
    ...
    <path pathLength={1} className="draw"
      ...
      strokeOpacity={0.8}
    />
    <path pathLength={1} className="draw" d={`M ${CX - 72} 8 Q ${CX} 21 ${CX + 72} 8`} strokeOpacity={0.28} />
    ...
    {/* Astragal */}
    <line pathLength={1} className="draw" x1={CX - 39} y1={106} x2={CX + 39} y2={106} strokeOpacity={0.5} />
  </g>
);
